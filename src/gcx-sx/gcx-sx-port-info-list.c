 ...
 * strings that are passed back by the libgcx-sx_port functions.
 *
 ...
 *
 * \return A non-negative number or a gcx-sx error code
 **/
int
...
foreach_func (const char *filename, lt_ptr data)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list",
 ...
 *
 * \return The number of entries or a gcx-sx error code
 **/
int
gp_port_info_list_count (GPPortInfoList *list)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list", _("Counting entries "
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list", _("%i regular entries "
		"available."), count);
	return (count);
}
 ...
 *
 * \return The index of the entry or a gcx-sx error code
 **/
int
gp_port_info_list_lookup_path (GPPortInfoList *list, const char *path)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list", _("Looking for "
	...
	for (i = 0; i < list->count; i++) {
		...
		if (result) {
			...
			if (regerror (result, &pattern, buf, sizeof (buf)))
				gp_log (GP_LOG_ERROR, "gcx-sx-port-info-list",
					"%s", buf);
			else
				gp_log (GP_LOG_ERROR, "gcx-sx-port-info-list",
					_("regcomp failed"));
...
gp_port_info_list_lookup_name (GPPortInfoList *list, const char *name)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list", _("Looking up entry "
		"'%s'..."), name);
...
gp_port_info_list_get_info (GPPortInfoList *list, int n, GPPortInfo *info)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port-info-list", _("Getting info of "
