...
/**
 * \brief Internal private libgcx-sx_port data.
...
gp_port_new (GPPort **port)
{
	CHECK_NULL (port);
        ...
        gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Creating new device..."));
...
gp_port_open (GPPort *port)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Opening %s port..."),
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Closing port..."));
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Freeing port..."));
 ...
 *
 * \return a negative gcx-sx error code or the amount of data written.
 **/
int
gp_port_write (GPPort *port, const char *data, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Writing %i=0x%x byte(s) "
		"to port..."), size, size);

	CHECK_NULL (port && data);
	CHECK_INIT (port);

	gp_log_data ("gcx-sx-port", data, size);
	...
	if ((port->type != GP_PORT_SERIAL) && (retval != size))
		gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Could only write %i "
 ...
 *
 * \return a gcx-sx error code or the amount of data read
 **/
int
gp_port_read (GPPort *port, char *data, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Reading %i=0x%x bytes from port..."),
		size, size);
	...
	CHECK_RESULT (retval);
	if (retval != size)
		gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Could only read %i "
			"out of %i byte(s)"), retval, size);

	gp_log_data ("gcx-sx-port", data, retval);
...
gp_port_check_int (GPPort *port, char *data, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Reading %i=0x%x bytes from interrupt endpoint..."),
		size, size);
...
gp_port_check_int_fast (GPPort *port, char *data, int size)
{
	...
	if ( retval != 0 ) {
		...
		gp_log (GP_LOG_DEBUG, "gcx-sx-port",
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Setting timeout to %i "
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Getting timeout..."));

	CHECK_NULL (port);

	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Current timeout: %i "
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Setting settings..."));
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Getting level of pin %i..."),
		pin);
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Level of pin %i: %i"),
		pin, *level);
...
gp_port_set_pin (GPPort *port, GPPin pin, GPLevel level)
{
	...
	for (j = 0; LevelTable[j].description; j++)
		if (LevelTable[j].level == level)
			break;
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Setting pin %i "
		"(%s: '%s') to '%s'..."), 
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Sending break (%i "
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Flushing port..."));
...
{
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Clear halt..."));
	...
	char *bytes, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Writing message "
		"(request=0x%x value=0x%x index=0x%x size=%i=0x%x)..."),
		request, value, index, size, size);
	gp_log_data ("gcx-sx-port", bytes, size);
	...
	char *bytes, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", _("Reading message "
		"(request=0x%x value=0x%x index=0x%x size=%i=0x%x)..."),
		request, value, index, size, size);
	...
	gp_log_data ("gcx-sx-port", bytes, retval);
	...
	int value, int index, char *bytes, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", "Writing message "
		"(request=0x%x value=0x%x index=0x%x size=%i=0x%x)...",
		request, value, index, size, size);
	gp_log_data ("gcx-sx-port", bytes, size);
	...
	char *bytes, int size)
{
	...
	gp_log (GP_LOG_DEBUG, "gcx-sx-port", "Reading message "
		"(request=0x%x value=0x%x index=0x%x size=%i=0x%x)...",
		request, value, index, size, size);
	...
	if (retval != size)
		gp_log (GP_LOG_DEBUG, "gcx-sx-port", "Could only read %i "
			"out of %i byte(s)", retval, size);
...
gp_port_set_error (GPPort *port, const char *format, ...)
{
	...
	if (format) {
		...
		vsnprintf (port->pc->error, sizeof (port->pc->error),
			   _(format), args);
		gp_log (GP_LOG_ERROR, "gcx-sx-port", "%s", port->pc->error);
		va_end (args);
	} else
		port->pc->error[0] = '\0';
