...
gp_port_usb_update (GPPort *port)
{
	...
	if (port->settings.usb.interface != port->pl->interface) {
		...
		if (usb_release_interface (port->pl->dh,
					   port->pl->interface) < 0) {
			gp_log (GP_LOG_DEBUG, "gcx-sx-port-usb","releasing the iface for config failed.");
			...
			ret = usb_claim_interface (port->pl->dh,
						   port->settings.usb.interface);
			if (ret < 0) {
				gp_log (GP_LOG_DEBUG, "gcx-sx-port-usb","reclaiming the iface for config failed.");
	...
	if (port->settings.usb.config != port->pl->config) {
		...
		if (ret < 0) {
			...
			return GP_ERROR_IO_UPDATE;	
#endif
			gp_log (GP_LOG_ERROR, "gcx-sx-port-usb","setting configuration from %d to %d failed with ret = %d, but continue...", port->pl->config, port->settings.usb.config, ret);
		}

		gp_log (GP_LOG_DEBUG, "gcx-sx-port-usb",
	...
	for (bus = usb_busses; bus; bus = bus->next) {
		...
		for (dev = bus->devices; dev; dev = dev->next) {
			...
			if ((dev->descriptor.idVendor == idvendor) &&
				...
				gp_log (GP_LOG_VERBOSE, "gcx-sx-port-usb",
				...
				if (dev->config) {
					...
					if (dev->config[config].interface[interface].altsetting[altsetting].bInterfaceClass
						...
						gp_log (GP_LOG_VERBOSE, "gcx-sx-port-usb",
							_("USB device (vendor 0x%x, product 0x%x) is a mass"
							  " storage device, and might not function with gcx-sx."
							  ),
							idvendor, idproduct);
					}
					...
					gp_log (GP_LOG_DEBUG, "gcx-sx-port-usb", "inep to look for is %02x", port->settings.usb.inep);
