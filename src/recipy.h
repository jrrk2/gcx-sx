...
/* flags telling what to add to recipy */
#define MKRCP_STD 1
#define MKRCP_TGT 2
...
/* a few helpers */
static inline int intval(GScanner *scan) {return (int)g_scanner_cur_value(scan).v_int;}
static inline double floatval(GScanner *scan) {return (double)g_scanner_cur_value(scan).v_float;}
static inline char * stringval(GScanner *scan) {return g_scanner_cur_value(scan).v_string;}

/* these shouldn't really be public, but report.c needs them */
GScanner* init_scanner();

struct stf * create_recipy(GSList *sl, struct wcs *wcs, int flags, 
