...
{
	gtk_object_set_data_full(GTK_OBJECT(window), "gui_star_list", 
				 gsl, (GtkDestroyNotify)gui_star_list_release);
}
			...
			GdkGC *gc, double zoom, GdkFont *font)
{
	...
	if ((gs->pair != NULL) && (gs->pair->flags & STAR_HAS_PAIR)) {
		...
		gdk_gc_destroy(gc_pline);
	}
	if ((gs->s != NULL) && (CAT_STAR(gs->s)->flags & INFO_POS)) {
		int pix, piy;
		pix = ix + (P_DBL(WCS_PLOT_ERR_SCALE) * 
		       CAT_STAR(gs->s)->pos[POS_DX] + 0.5) * zoom;
		piy = iy + (P_DBL(WCS_PLOT_ERR_SCALE) * 
		       CAT_STAR(gs->s)->pos[POS_DY] + 0.5) * zoom;
		gdk_draw_line(drawable, gc, ix, iy, pix, piy);
		gdk_draw_arc(drawable, gc, TRUE, pix-2, piy-2, 
			     4, 4, 0, 360*64);
	}
...
void draw_gui_star(struct gui_star *gs, GtkWidget *window)
{
	...
	draw_star_helper(gs, darea->window, gsl, gc, geom->zoom, darea->style->font);
	gdk_gc_destroy(gc);
}
...
//	d3_printf("find_stars_cb action %d\n", action);
	...
	case ADD_STARS_GSC:
		...
		if (wcs == NULL || wcs->wcsset == WCS_INVALID) {
			statusbar2_message(window, " Need an Initial WCS to Load GSC Stars", 
					   "sources", 5000);
			error_beep();
			return;
		}
	...
	case ADD_STARS_TYCHO2:
		...
		wcs = gtk_object_get_data(GTK_OBJECT(window), "wcs_of_window");
		if (wcs == NULL || wcs->wcsset == WCS_INVALID) {
			statusbar2_message(window, " Need an initial WCS to load Tycho2 stars", 
					   "sources", 5000);
			error_beep();
			return;
		}
	...
	case ADD_STARS_DETECT:
		...
		statusbar2_message(window, " Searching for stars...", "sources", 0);
...
/* now add to the list */
		...
		sprintf(buf, " Found %d stars (SNR > %.1f)", src->ns, P_DBL(SD_SNR));
	...
	case ADD_STARS_OBJECT:
		...
		wcs = gtk_object_get_data(GTK_OBJECT(window), "wcs_of_window");
		if (wcs == NULL || wcs->wcsset == WCS_INVALID) {
			statusbar2_message(window, " Need an Initial WCS to Load objects", 
					   "sources", 5000);
			error_beep();
			return;
		}
...
void select_stars(GtkWidget *window, GdkEventButton *event, int multiple)
{
	...
	while (sl != NULL) {
		gs = GUI_STAR(sl->data);
		sl = g_slist_next(sl);
		d3_printf("found [%08x] %.2f,%.2f  size %.0f\n", 
			  (unsigned)gs, gs->x, gs->y, gs->size) ;
...
//		}
	...
	if (sl == NULL) {
		statusbar2_message(window, " No star to unmark", "sources", 3000);
		error_beep();
	}
	gtk_widget_queue_draw(window);
}
...
static void try_attach_pair(GtkWidget *window, GSList *found)
{
	...
	if (pair == NULL) {
		statusbar2_message(window, " Cannot find suitable pair.", "sources", 3000);
		error_beep();
		return;
	}
	...
	statusbar2_message(window, " Creating Star Pair", "sources", 2000);
...
static void sprint_star(char *buf, int len, struct gui_star *gs, struct wcs *wcs)
{
	...
	if ((TYPE_MASK_GSTAR(gs) & TYPE_MASK_CATREF) && (gs->s)) {
		degrees_to_dms_pr(ras, CAT_STAR(gs->s)->ra / 15.0, 0);
		degrees_to_dms_pr(decs, CAT_STAR(gs->s)->dec, 0);
	...
	if (wcs == NULL || wcs->wcsset == WCS_INVALID) {
		...
		degrees_to_dms(ras, xpos / 15.0);
		degrees_to_dms(decs, ypos);
...
void star_display_cb(gpointer window, guint action, GtkWidget *menu_item)
{
	...
	case STAR_FAINTER:
		...
		snprintf(msg, 128, " Display limiting magnitude is %.1f", P_DBL(DO_DLIM_MAG));
		statusbar2_message(window, msg, "stars", 5000);
		break;
	case STAR_BRIGHTER:
...
void star_edit_cb(gpointer window, guint action, GtkWidget *menu_item)
{
	...
	if (sl == NULL) {
		error_message_sb2(window, " No stars selected");
		return;
	}
