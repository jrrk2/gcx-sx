...
static int find_index(const char *tycho, // same as above, the files of the catalog
	...
	fread(radi, sizeof(long), 3600, raf);
	fclose(raf);
	...
	fread(decdi, sizeof(long), 3600, decf);
	fclose(decf);
	...
	fread(rai, sizeof(IXENT), rail, raf);
	fclose(raf);
	...
	fread(deci, sizeof(IXENT), decil, decf);
	fclose(decf);
		  ...
		  char *buf, int sz, char *path)
{
	...
	asprintf(&raix, "%s/ra.ix", dir);
	asprintf(&raixx, "%s/ra.ixx", dir);
	asprintf(&decix, "%s/dec.ix", dir);
	asprintf(&decixx, "%s/dec.ixx", dir);
...
//		d3_printf("Remaking indices\n");
	...
	retl = find_index(path, raix, decix, raixx, decixx,
			      ra-w/2, ra+w/2, dec-h/2, dec+h/2, CAT_GET_SIZE, rets);
