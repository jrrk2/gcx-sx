...
static int rep_aavso_star(FILE *repfp, struct star_obs *sob)
{
	char line[256];
	int i, nl;
	char *sequence;
...
static int rep_star(FILE *repfp, struct star_obs *sob, int format)
{
	switch(format) {
	case REP_AAVSO:
		return rep_aavso_star(repfp, sob);
		break;
	}
	return -1;
}
		...
		for (fl = ofrs; fl != NULL; fl = g_list_next(fl)) 
			for (sl = O_FRAME(fl->data)->sol; sl != NULL; sl = g_list_next(sl)) {
				if (CATS_TYPE(STAR_OBS(sl->data)->cats) != 
					...
					continue;
				n += !rep_star(repfp, STAR_OBS(sl->data), 
					       action & FMT_MASK);
			}
		return n;
		break;
...
char * mbds_short_result(struct o_frame *ofr)
{
	...
	for (sl = ofr->sol; sl != NULL; sl = g_list_next(sl)) {
		if (CATS_TYPE(STAR_OBS(sl->data)->cats) != 
		    CAT_STAR_TYPE_APSTAR)
			continue;
		asprintf(&rep, "%s: %s=%.3f/%.3f  %d outliers, fit me1=%.2g",
