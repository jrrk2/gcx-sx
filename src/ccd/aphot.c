...
static double hist_clip_avg(struct rstats *rs, double *median, double sigmas)
{
	int ix, it=0;
	int iy = 0;
	int all = 0, n;
	double m=0, mo, s2, sum;
	int s=0, e=H_SIZE;
	...
	do {
		...
		if (e > H_SIZE)
			e = H_SIZE;
		d4_printf("hist clip: [%d %d]\n", s+H_START, e+H_START);
		it ++;
	
	} while (it < 20 && (it == 1 || fabs(mo - m) > 0.00001));
...
//	d3_printf("ringstats avg:%.2f sum:%.2f sigma%.2f, min%.2f max%.2f dx%.2f dy%.2f", 
//		  rs->avg, rs->sum, rs->sigma, rs->min, rs->max, rs->dx, rs->dy); 
	return 0;
}
	       ...
	       struct ap_params *p, struct bad_pix_map *bp)
{
	struct rstats rs;
	int ret, i;
	double phn, omed, med, min, max, flnsq;
	double mean, median;
...
//		do {
			med = rs.median;
			min = med - p->sigmas * rs.sigma;
			max = med + p->sigmas * rs.sigma;
			ret = ring_stats(fr, s->x, s->y, p->r2, p->r3, p->quads, &rs, min, max);
			if (ret) {
				err_printf("ap_get_sky: error in ring_stats\n");
				return ret;
			}
//		} while(i < p->max_iter && rs.median != med);
//		if (i == p->max_iter) {
//			d3_printf("mmedian did not converge after %d iterations %.1f, %.1f\n",
//				  omed, rs.median); 
//			s->flags |= AP_DID_NOT_CONVERGE;
//		}
//		d3_printf("sky omed: %.1f, med:%.1f, mm: %.1f\n", omed, med, rs.avg);
		s->aph.sky = rs.avg;
		break;
	case APMET_KS:
		s->aph.sky = hist_clip_avg(&rs, NULL, p->sigmas);
		break;
	case APMET_SYMODE:
		d4_printf("SYMODE--");
		mean = hist_clip_avg(&rs, &median, p->sigmas);
				...
				double *cxerr, double *cyerr)
{
	...
	if (n == 0 || sum == 0) {
		...
		if (cy)
			*cy = sy / sum;
		if (cxerr)
			*cxerr = sqrt((xsq / sum - sqr(sx / sum)) / sum);
		if (cyerr)
			*cyerr = sqrt((ysq / sum - sqr(sy / sum)) / sum);
	}
	       ...
	       struct ap_params *p, struct bad_pix_map *bp)
{
	...
	s->aph.tflux = aperture_flux(fr, p->r1, s->x, s->y, 
			&s->aph.star_min, &s->aph.star_max, &s->aph.star_all, &fsq);
...
// copy stats to star structure
	s->aph.tflux = rs.sum;
	s->aph.star_min = rs.min;
	s->aph.star_max = rs.max;
	s->aph.star_all = rs.used;

// see if any flags need to be set
	if (rs.used != rs.all)
		s->aph.flags |= AP_STAR_SKIP;
	if (rs.max > p->sat_limit)
		s->aph.flags |= AP_BURNOUT;
	else if (rs.max > p->sat_limit / 2)
		s->aph.flags |= AP_BRIGHT;
	...
	s->aph.pshot_noise = phn;
	s->aph.rd_noise = fr->exp.rdnoise * sqrt(s->aph.star_all);
//	d3_printf("pshotn %.4f fluxerr %.4f flat %.4f\n", phn, s->flux_err, sqrt(flnsq));
	return 0;
}
...
// compute star flux and error
	...
	if (cxerr < BIG_ERR && rm > 1) 
		s->xerr = cxerr * sqrt(rm) / s->aph.star_err;
	if (cyerr < BIG_ERR && rm > 1) 
		s->yerr = cyerr * sqrt(rm) / s->aph.star_err;
...
// clip to protect the logarithms
	...
	s->aph.absmag = -2.511886*log10(s->aph.star);
...
// temporary magerr that doesn't include scintillation
	s->aph.magerr = 2.511886 * (log10(s->aph.star + s->aph.star_err) - log10(s->aph.star));
