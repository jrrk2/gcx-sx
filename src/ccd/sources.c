...
#define	MINSEP		10		// min star separation
...
// walk the ring and collect statistics
	...
	for (iy = ys; iy < ye; iy++) {
		dy2 = (iy - yc) * (iy - yc);
		for (ix = xs; ix < xe; ix++) {
			if (dp >= ((float *)(fr->dat)) + all) {
				err_printf("scanning past end of frame!\n");
				break;
			}
			v = *dp;
			dp ++;
...
// first, find the stats for the search region
	...
	if (fr->stats.statsok && fr->stats.csigma < reg.sigma) {
		minpk = reg.median + 2 * NSIGMA * fr->stats.csigma;
	} else {
...
// we finally have it; fill up return values and exit
			...
			if (m.sum < 1) {
				...
				s->xerr = sqrt((m.mx2 - sqr(m.mx))/(m.sum));
				s->yerr = sqrt((m.my2 - sqr(m.my))/(m.sum));
			}
...
int extract_stars(struct ccd_frame *fr, struct region *reg, double min_flux, double sigmas, struct sources *src)
{
	...
	if (reg != NULL) {
		err_printf("extract_stars: i dont know how to do regions yet\n");
	}
