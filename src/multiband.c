...
static void ofr_sob_initial_weights(struct o_frame *ofr, struct transform *trans)
{
	...
	while(sl != NULL) {
		...
		sl = g_list_next(sl);
		sob->nweight = 0.0;
//		d3_printf("%s flags: %08x\n", sob->ost->cats->name, sob->ost->cats->flags);
		...
		if (trans != NULL) {
			if(sob->ost->smagerr[trans->c1] >= BIG_ERR)
				continue;
			if(sob->ost->smagerr[trans->c2] >= BIG_ERR)
				continue;
		      ...
		      double alpha, double beta, int w_res)
{
	...
	if ((ofr->vstars > 1) && (ofr->tweight > 0)) {
		ofr->zpointerr = ofr->me1 / sqrt((ofr->tweight / (ofr->vstars - 1)));
...
//		d3_printf("tok: %d state: %d start: %s end: %s\n", tok, state, start, end);
		...
		case 0: 	/* wait for band */
			...
			if (tok != TOK_WORD) 
				break;
			strncpy(buf, start, end - start > 63 ? 63 : end - start);
...
int gaussj(double a[MAX_MBANDS + 1][MAX_MBANDS + 1], int n, double b[MAX_MBANDS + 1]) 
...
/* We now have the pivot element, so we interchange rows, 
   ...
   matrix will be scrambled by columns. */
		...
		indxc[i]=icol; 
		if (a[icol][icol] == 0.0) {
			d3_printf("gaussj: singular matrix\n");
			return -1;
		}
...
/*
	...
	d3_printf("\n");
*/
	if (gaussj(a, MAX_MBANDS, b) /* || b[sob->ofr->band + 1] == 0*/) {
		d3_printf("gausj error\n");
		goto nocol;
	}
...
void ofr_to_stf_cats(struct o_frame *ofr)
{
	...
	for (sl = ofr->sol; sl != NULL; sl = sl->next) {
		...
		if (!(CATS_TYPE(sob->cats) == CAT_STAR_TYPE_APSTD)) {
			...
			sob->cats->stderr = fabs(sob->residual * sqrt(sob->nweight));
