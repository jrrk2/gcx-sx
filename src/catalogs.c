...
//		d3_printf("tycho star is: \n %s\n", p);
		...
		if (vt < 30.0 && bt < 30.0) {
			...
			cats->mag = vt;
			asprintf(&cats->smags, "vt=%.2f/%.3f", vt, vterr);
		} else {
			cats->mag = bt;
			asprintf(&cats->smags, "bt=%.2f/%.3f", bt, bterr);
		}
		st[i] = cats;
		p+= TYCRECSZ + 1;
	}
...
/* we dont really need a close, but may later */
...
int update_band_by_name(char **mags, char *band, double mag, double err)
{
	...
	if (*mags == NULL) {
		if (err == 0.0)
			...
			asprintf(&nb, "%s=%.3f/%.3f", band, mag, err);
		*mags = nb;
		return 0;
	}
	...
	if (bs == NULL) {
		if (err == 0.0)
			asprintf(&nb, "%s %s=%.3f", *mags, band, mag);
		else 
			asprintf(&nb, "%s %s=%.3f/%.3f", *mags, band, mag, err);
		if (*mags)
			free(*mags);
		*mags = nb;
		return 0;
