...
/* create/read recipy files; read other star list files */
...
/*
static inline void put_indent(int indent, int indstart, FILE *fp)
{
	int i;
	for (i = 0; i < indent * INDENT - indstart; i++)
		fputc(' ', fp);
}


static void recipy_report_head(FILE *fp, struct wcs *wcs, char *comment)
{
	char decs[64];
	char ras[64];
	...
	if (wcs->wcsset < WCS_INITIAL) {
		err_printf("Bad wcs: skipping field center reporting in rcp\n");
		if (comment != NULL)
			fprintf(fp, "recipy (comments \"%s\")\n", comment);
		else
			fprintf(fp, "recipy ( )\n");
	} else {
		degrees_to_dms_pr(ras, wcs->xref / 15.0, 1);
		degrees_to_dms_pr(decs, wcs->yref, 0);
		fprintf(fp, "recipy (ra \"%s\" dec \"%s\" equinox %d",
			ras, decs, wcs->equinox);
...
static void catalog_report_head(FILE *fp, char *comment)
{
	fprintf(fp, "(");
	put_indent(1, 1, fp);
	if (comment != NULL)
		fprintf(fp, "catalog (comments \"%s\")\n", comment);
	else
		fprintf(fp, "catalog ( )\n");
	put_indent(1, 0, fp);
	fprintf(fp, "stars (\n");
...
/* report a single cat star. return 1 if the star was reported successfully, 
 * 0 if it was skipped for some reason */
/*
static int recipy_report_cat_star(FILE *fp, struct cat_star *cats, int indent, int type, 
				  double rcp_equinox)
{
	char ras[64], decs[64];
	char * typestr;
	...
	case MKRCP_CAT:
		...
		break;
	default:
		d3_printf("ignoring star with unknown type %d in recipy_report_cat_star\n", 
			  type);
		return 0;
	}

	put_indent(indent, 2, fp);
	degrees_to_dms_pr(ras, cats->ra / 15.0, 2);
	degrees_to_dms_pr(decs, cats->dec, 1);
	if (cats->equinox != rcp_equinox) 
		fprintf(fp, "( name \"%s\" type %s ra \"%s\" dec \"%s\" equinox %.0f",
			cats->name, typestr, ras, decs, cats->equinox);
	else
		fprintf(fp, "( name \"%s\" type %s ra \"%s\" dec \"%s\"",
			cats->name, typestr, ras, decs);
	if (cats->comments != NULL && cats->comments[0] != 0) {
		fprintf(fp, "\n");
		put_indent(indent, 0, fp);
		fprintf(fp, "comments \"%s\"", cats->comments);
	}
	if (type != MKRCP_TGT) {
		fprintf(fp, "\n");
		put_indent(indent, 0, fp);
		if (cats->mag != 0.0 && cats->smags != NULL)
			fprintf(fp, "mag %.1f smags \"%s\" ",
				cats->mag, cats->smags);
		else if (cats->mag != 0.0)
			fprintf(fp, "mag %.1f ",
				cats->mag);
		else if (cats->smags != NULL)
			fprintf(fp, "smags \"%s\" ",
				cats->smags);
	} else {
		if (cats->mag != 0.0) {
			...
			fprintf(fp, "mag %.1f ",
				cats->mag);
		} else if (cats->flags & (CAT_ASTROMET | CAT_VARIABLE)) {
			fprintf(fp, "\n");
			put_indent(indent, 0, fp);
		}
	}
	if (cats->flags & (CAT_ASTROMET | CAT_VARIABLE)) {
		fprintf(fp, "flags (");
		if (cats->flags & CAT_ASTROMET)
			fprintf(fp, " astrom");
		if (cats->flags & CAT_VARIABLE)
			fprintf(fp, " var");
		fprintf(fp, " ) )\n");
	} else {
		fprintf(fp, ")\n");
...
/* do the actual work of creating recipy. sl is the list of gui_stars */
struct stf * create_recipy(GSList *gsl, struct wcs *wcs, int flags, 
...

{
	...
	if (g_list_length(tsl) == 0) {
		err_printf("No stars for recipy\n");
		return NULL;
	}
	...
	stf = stf_append_list(NULL, SYM_RECIPY, st);
	if (seq != NULL && seq[0] != 0) {
...
/* read a recipy an return the number of stars read. Place pointers to newly-allocated
...
/* convert an old style recipy to new. Return the number of stars written */
/* TODO: convert to stf style */
#define MAX_STARS_CONV 100000
int convert_recipy(FILE *inf, FILE *outf) 
{
	return -1;
/*
	...
	recipy_report_head(outf, &wcs, NULL);

	for (i=0; i<n; i++) {
		if (CATS_TYPE(csl[i]) == CAT_STAR_TYPE_APSTD)
			ret += recipy_report_cat_star(outf, csl[i], 2, MKRCP_STD, 
						      1.0 * wcs.equinox);
	}
	for (i=0; i<n; i++) {
		if (CATS_TYPE(csl[i]) == CAT_STAR_TYPE_APSTAR)
			ret += recipy_report_cat_star(outf, csl[i], 2, MKRCP_TGT, 
						      1.0 * wcs.equinox);
	}
	for (i=0; i<n; i++) {
		if (CATS_TYPE(csl[i]) == CAT_STAR_TYPE_SREF)
			ret += recipy_report_cat_star(outf, csl[i], 2, MKRCP_FIELD, 
						      1.0 * wcs.equinox);
	}
	for (i=0; i<n; i++) {
...
int read_landolt_table(struct cat_star *csl[], FILE *fp, int n, double *cra, double *cdec)
{
	...
	while ( (sn < MAX_STARS_READ)) {
		...
		cats->mag = vmag;
		cats->equinox = 2000.0;
		asprintf(&cats->smags, "v=%.3f/0.005 b=%.3f/0.005 "
			 "u=%.3f/0.005 r=%.3f/0.005 i=%.3f/0.005",
			 vmag, bmag, umag, rmag, imag);
		cats->flags = CAT_STAR_TYPE_APSTD | CAT_ASTROMET;
		asprintf(&cats->comments, "%s", nam);
		cst[sn]=cats;
		sn ++;
	}
	for (i=0; i<n && i<sn; i++)
...
static int read_gcvs_table(struct cat_star *csl[], FILE *fp, int n, double *cra, double *cdec)
{
	...
	while ( (sn < MAX_STARS_READ)) {
		...
		if (spec[0] != 0 && !amp) {
			asprintf(&cats->comments, "p=V t=%s s=%s m(%s)=%.2f/%.2f",
				 type, spec, phot, mmax, mmin);
		} else if (!amp) {
			asprintf(&cats->comments, "p=V t=%s m(%s)=%.2f/%.2f",
				 type, phot, mmax, mmin);
		} else if (spec[0] != 0 && amp) {
			asprintf(&cats->comments, "p=V t=%s s=%s m(%s)=%.2f(%.3f)",
				 type, spec, phot, mmax, mmin);
		} else {
			asprintf(&cats->comments, "p=V t=%s m(%s)=%.2f(%.3f)",
				 type, phot, mmax, mmin);
		}
...
/* convert a catalog table file to recipy. Return the number of stars written 
...
/* merge the specified target object into the recipy and write it out on outf;
   mag_limit is ignored here */
int rcp_set_target(FILE *old, char *obj, FILE *outf, double mag_limit)
{
	...
	if (st == NULL) { 	/* we prepend the "recipy" alist here */
		st = stf_new();
		st->next = stf_new();
		st->next->next = stfa;
		stfa = st;
